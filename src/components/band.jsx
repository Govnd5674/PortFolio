...
export default function Lanyard({ position = [0, 30, 30], gravity = [0, -40, 0], fov = 20, transparent = true }) {
  const [isLoading, setIsLoading] = useState(true);
  const [initialGravity, setInitialGravity] = useState([0, 0, 0]);

  useEffect(() => {
    // Reduce initial delay to 300ms
    const timer = setTimeout(() => {
      setIsLoading(false);
      // Gradually increase gravity over 500ms
  ...
  return (
    <div className={`relative z-0 w-full h-screen flex justify-center items-center transform scale-100 origin-center transition-opacity duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
      <Canvas
  ...
  return (
    <>
      <group position={[4, 4, 0]}>
        <RigidBody ref={fixed} position={[0, 0, 0]} {...segmentProps} type="fixed" />
